...
####################################################################################################################################################################################################################################
# Shared variables (and mutex) to allow inter-thread communication #
####################################################################
...
def callback (key) :
    ...
    # We set the decision and allow the player to process the decision
    global decision
    global decisionMutex
    if decisionMutex.locked() :
        decision = key
        decisionMutex.release()

####################################################################################################################################################################################################################################
# GUI thread that captures the keyboard strokes #
#################################################
...
def interfaceThread () :
    ...
    # Buttons
    biggerFont = tkinter.font.Font(size=30)
    topButton = tkinter.Button(frame, text="▲", height=1, width=2, font=biggerFont, command=(lambda : callback(MOVE_UP)))
    topButton.grid(row=0, column=1)
    leftButton = tkinter.Button(frame, text="◀", height=1, width=2, font=biggerFont, command=(lambda : callback(MOVE_LEFT)))
    leftButton.grid(row=1, column=0)
    bottomButton = tkinter.Button(frame, text="▼", height=1, width=2, font=biggerFont, command=(lambda : callback(MOVE_DOWN)))
    bottomButton.grid(row=1, column=1)
    rightButton = tkinter.Button(frame, text="▶", height=1, width=2, font=biggerFont, command=(lambda : callback(MOVE_RIGHT)))
    rightButton.grid(row=1, column=2)
    
    # Keys
    frame.bind("<Up>", lambda event : callback(MOVE_UP))
    frame.bind("<Left>", lambda event : callback(MOVE_LEFT))
    frame.bind("<Down>", lambda event : callback(MOVE_DOWN))
    frame.bind("<Right>", lambda event : callback(MOVE_RIGHT))
    
    # Go
    frame.mainloop()

####################################################################################################################################################################################################################################
# Function that enables or disables key repetition to avoid unwanted keypresses during the game #
#################################################################################################

def setKeyRepetition (enable) :
    
    # System call to enable/disable keypress repeating
    mode = "on" if enable else "off"
    os.system("xset r " + mode)
...
def preprocessing (mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, piecesOfCheese, timeAllowed) :
    ...
    # We disable keyboard repetition for the duration of the game
    atexit.register(setKeyRepetition, True)
    setKeyRepetition(False)
    
    # We start the interface allowing the user to press keys
    thread = threading.Thread(target=interfaceThread)
    thread.start()
    
    # We start with the mutex already locked
    global decisionMutex
...
def turn (mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, playerScore, opponentScore, piecesOfCheese, timeAllowed) :
    ...
    # We return the decision when it is available
    global decisionMutex
    decisionMutex.acquire()
    return decision
